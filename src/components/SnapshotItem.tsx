...
export function SnapshotItem({ snapshot, projectName, selected, onClick }: SnapshotItemProps) {
  ...
  const handleMerge = (e: React.MouseEvent) => {
    e.stopPropagation();
    setMergeSnapshotModalOpen(true, {
      projectName,
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="ml-auto flex items-center gap-1 opacity-0 group-hover:opacity-100 flex-shrink-0">
              <button
                ...
                onClick={handleMerge}
                title="Merge snapshot into main"
              >
                <GitMerge size={12} />
              </button>
              <button
                className="p-1 rounded text-envibe-text-muted hover:text-envibe-danger hover:bg-envibe-danger/20 transition-colors"
